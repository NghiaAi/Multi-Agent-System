...
def run_trading_agent(query: str, sql_result: list = None) -> dict:
    ...
    # If only "Adj Close" is missing but "Close" exists, use Close as a proxy
    if "Adj Close" in missing and "Close" in df.columns:
        df["Adj Close"] = df["Close"]
        missing.remove("Adj Close")

    # If still missing any core columns, return a clear error instead of crashing
    if any(col in ["Open", "High", "Low", "Close", "Volume"] for col in missing):
        return {
            "status": "error",
            "message": (
                "SQL result for trading must include columns: "
                "'Date', 'Open', 'High', 'Low', 'Close', 'Adj Close', 'Volume', 'Ticker'. "
                "Please adjust the Text-to-SQL query accordingly."
            ),
        }
